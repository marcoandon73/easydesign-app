import { Body, Controller, Delete, Get, Post, Put, Query, Res, UploadedFile, UseGuards, UseInterceptors, UsePipes } from '@nestjs/common';
...
@Controller('bg_categories')
export class BgCategoriesController {
  constructor(
    private categoriesService: BgCategoriesService,
  ...
  async deleteOne(@Body() params: DeleteCategoryDto, @User() user: UsersEntity, @Res() res) {
    ...
    try {
      ...
      console.log('error delete one function', e);
      if (e.status === 500 || !e?.status) {
        return res.status(500).send({ message: 'Internal server error', status: 500 });
      } else {
        return res.status(e.status).send(e);
